...
5장. '데이터베이스' 조작이 편해지는 'ORM'
            ...
            관계형 DBMS
               ...
               H2, MySQL
                    ...
                    H2 - 자바로 작성되어 있는 RDBMS : 스프링 부트가 지원하는 인메모리 관계형 데이터베이스
                        ...
                        그래서 애플리케이션 재실행시 데이터는 초기화됨(서버 재실행 기준으로).
                ...
                필수 용어
                    ...
                    3) 열(column) : 테이블의 구성 요소 중 하나이며, 행에 저장되는 유형의 데이터.
                        예를 들어 회원 테이블이 있다고 할 때, 열은 각 요소에 대한 속성을 나타내며(Java에서 field)
                        무결성을 보장함. 이상의 표를 기준으로 할 때는, 이메일은 문자열일 것, 나이는 숫자 유형을 가짐.
                        이메일 열에 숫자가 들어가거나, 나이 열에 문자열이 들어갈 수 없기 때문에 데이터에 대한 무경성을 보장.
    ...
    2. ORM
        ...
        장점 :
            ...
            3) 데이터베이스 시스템 추상화되어있기 때문에 MySQL에서 PostgreSQL로 전환을 하더라도 추가로 드는 작업이 거의 없음.
